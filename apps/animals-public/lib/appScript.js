// Sibling of the reptile/amphibian calendar's apps/reptile-public/lib/appScript.js,
// itself descended from the bird app's engine. Non-chordate animal families
// DO split cleanly into a fixed 12 groups by phylum/class (see PHYLUM_TO_GROUP
// in scripts/build_animal_families_from_backbone.py) -- so this reuses the
// same admin-editable engine as reptile/bird rather than hardcoding a month
// table the way the mammal app's CLADES does: ORDER_DATA
// (data/animals/orders.json, admin-editable) carries a `month` per group
// that starts out null -- a family whose group has no month yet simply has
// no date, and stays out of the calendar, search's month tag, and the .ics
// export until it does.
//
// Unlike mammal/bird/reptile, there's no species epithet here -- this is a
// FAMILY-level calendar, so every place those apps' day/hour/minute letter
// math ran on a species name, this runs on the family name itself instead.
// eslint-disable
import { ORDER_EMOJI } from "./orderEmoji";

export function initAnimalCalendarApp(FAMILIES_DATA, ORDER_DATA, INITIAL_FAQS, BROWSE_PROMPT, MONTH_DESCRIPTIONS) {
"use strict";
  ...
  // ---------- Family data ----------
  // Sourced from GBIF's backbone taxonomy, filtered to non-chordate Animalia
  // -- see scripts/build_animal_families_from_backbone.py for exactly how
  // each of the 12 groups was resolved and how common names were derived.
  // See data/animals/families.json.
  var FAQS = INITIAL_FAQS.slice();

  // [common name, Family, orderIndex] -- no species epithet at this level.
  var DATA = FAMILIES_DATA;

  // Unlike the flowering-plant app, there's no curated per-family emoji
  // data for non-chordate animal families (13,030 of them, overwhelmingly
  // obscure) -- so each month card's badge is drawn from a plain per-group
  // pool (lib/orderEmoji.ts), same as the bird and reptile/amphibian apps.
  ...
  // the family name itself rather than a species epithet) ----------
  function letterIndex(ch){ return ch.toUpperCase().charCodeAt(0) - 64; } // A=1..Z=26

  // Day comes from the family name's first letter (1-26) UNLESS the name
  ...
  function dayOf(family, month){
    var name = family.toUpperCase();
    ...
    if (month === 1) return letterIndex(family[0]); // February stops here
    if (name.indexOf("PP") !== -1) return 29;
    if (name.indexOf("RR") !== -1) return 30;
    if (MONTH_DAYS[month] === 30) return letterIndex(family[0]); // 30-day months stop here
    if (name.indexOf("SS") !== -1) return 31;
    return letterIndex(family[0]);
  }

  // Hour and minute both come from the family name: add up the letter
  ...
  //
  // Family names run much longer than a species epithet ever did (some top
  // 20 letters -- confirmed live: the 25-letter Parallelepipedorhynchidae),
  // long enough for the raw hour digits to occasionally exceed 23 and
  // produce an impossible clock time. Wrapping with % 24 keeps the exact
  // same letters-to-time rule for every name that was already in range
  // (mod is a no-op there) while guaranteeing a valid 24-hour time for
  // every family, however long its name is.
  function hourMinuteOf(family){
    var sum = 0;
    for (var i = 1; i < family.length; i++) sum += letterIndex(family[i]);
    var str = String(sum);
  ...
  // Returns null fields when the family' group has no month yet -- callers
  ...
  function compute(entry){
    ...
    if (month == null){
      return { common: entry[0], family: family, order: order, month: null, day: null, hour: null, minute: null };
    }
    var day = dayOf(family, month);
    var hm = hourMinuteOf(family);
    return {
      common: entry[0], family: family,
      order: order, month: month,
      day: day, hour: hm.hour, minute: hm.minute
    };
  }

  // Families with a real date, precomputed once -- everything that scans by
  ...
  function refreshAssigned(){
    ASSIGNED = DATA.filter(function(entry){ return ORDERS[entry[2]].month != null; });
  }
  refreshAssigned();
  ...
  function buildMonthGrid(){
    for (var month = 0; month < 12; month++){
      (function(month){
        ...
        dayGrid.setAttribute("aria-label", MONTH_NAMES[month] + ", " + monthDays + " days, shaded by family count -- click a day to browse it");
  ...
  // ---------- Family-count heat map ----------
  // Each day box is shaded relative to that MONTH's own busiest day, so
  // every group's shape is visible regardless of the group's overall size.
  ...
  function paintHeatmap(){
    MONTHS.forEach(function(m, month){
      ...
      Object.keys(m.cells).forEach(function(d){
        ...
        cell.title = count
          ? MONTH_NAMES[month] + " " + day + " (" + letterForDay(day) + ") — " + count + " families"
  ...
  function renderResults(result){
    ...
    if (!matches.length){
      ...
      d.textContent = "No family matches that name.";
  ...
  // A bare family article is frequently a short taxobox stub with no photo
  // -- especially for one of the thousands of obscure, little-known
  // families here, where Wikipedia keeps the real, fully-illustrated
  // article under the common name instead (Cirroteuthidae's photo lives on
  // "Octopus", not "Cirroteuthidae"). So: try the family title first, then
  // the common name as a direct title (a precise, redirect-following
  // lookup -- no relevance ranking involved), and only as a last resort
  // fall back to Wikipedia's full-text search.
  //
  // That search step is deliberately keyed on the COMMON name, never the
  // family: an -idae name is Latin taxonomic jargon that full-text search
  // frequently can't match to anything relevant at all, or only to a
  // passing mention buried in an unrelated genus or species article --
  // "Octopus" reliably surfaces the real, photo-rich article, where
  // searching "Cirroteuthidae" itself would not.
  ...
  function loadSpecimenPhoto(r){
    ...
    function show(title, data){
      ...
      img.alt = "Photo of " + r.common + " (" + r.family + ")";
      img.src = src;
    ...
    function tryCommonNameThenSearch(){
      var hasDistinctCommonName = r.common && r.common.toLowerCase() !== r.family.toLowerCase();
      ...
      afterCommonName.then(function(commonData){
        ...
        // Neither the family nor the common name is a page with its own
        // photo -- last resort is Wikipedia's own full-text search,
        // keyed on the common name (see note above on why never the
        // family).
        var searchQuery = hasDistinctCommonName ? r.common : r.family;
    ...
    fetchSummary(r.family).then(function(data){
      if (reqId !== photoRequestSeq) return;
      if (show(r.family, data)) return;
  ...
  function buildIcs(r, wikiUrl){
    ...
    var descLines = [r.family, wikiUrl];

    var uid = encodeURIComponent(r.family) + "@animalfamilyephemeris";
    var lines = [
      "BEGIN:VCALENDAR",
      "VERSION:2.0",
      "PRODID:-//The Animal Family Ephemeris//EN",
      "CALSCALE:GREGORIAN",
      "BEGIN:VEVENT",
      "UID:" + uid,
      "DTSTAMP:" + dtstamp,
  ...
  function downloadIcs(entry){
    ...
    // for this entry right now -- the resolved common-name page once photo
    // lookup finds one, the bare family page otherwise.
    var wikiUrl = currentWikiUrl || ("https://en.wikipedia.org/wiki/" + encodeURIComponent(r.family));
  ...
  function selectEntry(entry, isAuto){
    ...
    document.getElementById("outSci").textContent = r.family;
    ...
    currentWikiUrl = "https://en.wikipedia.org/wiki/" + encodeURIComponent(r.family);
    loadSpecimenPhoto(r);
  }
  ...
  // is an instant lookup and never lists a family with no date.
  var DATE_INDEX = [];
  for (var mi = 0; mi < 12; mi++){
    DATE_INDEX[mi] = [];
  ...
  function renderBrowse(month, day){
    ...
    browseSummary.innerHTML = list.length
      ? "<b>" + list.length + "</b> families share " + MONTH_NAMES[month] + " " + day + " (" + letterForDay(day) + ")."
  ...
  // ---------- Auto-select today's closest family ----------
  // Show whichever family lands on today's real month/day (among families
  // whose group has been assigned one) and has the hour:minute closest to
  // right now. If nobody lands on today -- likely, until enough groups are
