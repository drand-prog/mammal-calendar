// Sibling of the mammal calendar's apps/public/lib/appScript.js, adapted for
// a taxon that doesn't split cleanly into 12 groups: birds have 46 orders,
// not 12 clades, so there's no fixed order-to-month table baked in here the
// way CLADES is in the mammal version. Instead ORDER_DATA (data/orders.json,
// admin-editable) carries a `month` per order that starts out null -- a
// species whose order has no month yet simply has no date, and stays out of
// the calendar, search's month tag, and the .ics export until it does.
// eslint-disable
...
"use strict";
  ...
  // ---------- Species data ----------
  // Sourced from AviList v2025b (June 2025), the taxonomy jointly adopted by
  // the IOC, Clements/eBird, and BirdLife/HBW committees -- every currently-
  // recognized extant species. See data/species.json.
  ...
  // Day comes from the species name's first letter (or, for names starting
  // "A" + a second letter A-E, a two-letter overflow code reaching the days
  // a single letter can't: AA=27, AB=28, AC=29 [a month's leap day, when it
  // has one], AD=30 [or the 28th in February], AE=31 [or the 28th/30th]).
  ...
  // values (A=1..Z=26) of every letter after the first one (or after the
  // overflow code's second letter, for an AA-AE name), then split that
  // sum's digits -- the last digit is the minute, whatever's left is the
  // hour. A sum under 10 has nothing to its left, so it reads as 0:0N.
  ...
  function buildIcs(r, wikiUrl){
    ...
    var uid = encodeURIComponent(r.genus + "_" + r.species) + "@birdephemeris";
    var lines = [
      "BEGIN:VCALENDAR",
      "VERSION:2.0",
      "PRODID:-//The Bird Ephemeris//EN",
      "CALSCALE:GREGORIAN",
      "BEGIN:VEVENT",
      "UID:" + uid,
      "DTSTAMP:" + dtstamp,
  ...
  function selectEntry(entry, isAuto){
    ...
    document.getElementById("outOrder").textContent = r.order.formal + " — " + r.order.name;
  ...
  function renderBrowse(month, day){
    ...
    browseSummary.innerHTML = list.length
      ? "<b>" + list.length + "</b> bird" + (list.length === 1 ? "" : "s") + " share " + MONTH_NAMES[month] + " " + day + " (" + letterForDay(day) + ")."
  ...
  // ---------- Auto-select today's closest bird ----------
