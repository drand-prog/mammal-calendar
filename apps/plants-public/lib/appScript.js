...
// itself descended from the bird app's engine. Flowering plant genera DO
// split cleanly into a fixed 12 groups (see GROUPS in
// scripts/wcvp_group_mapping_diagnostic.py) -- so this reuses the same
// admin-editable engine as reptile/bird rather than hardcoding a month
// table the way the mammal app's CLADES does: ORDER_DATA
// (data/plants/orders.json, admin-editable) carries a `month` per group
// that starts out null -- a genus whose group has no month yet simply has
...
"use strict";
  ...
  // for exactly how each of the 12 groups was resolved and how common
  // names were derived. See data/plants/genera.json.
  ...
  // Day comes from the genus name's first letter (1-26) UNLESS the name
  ...
  function dayOf(genus, month){
    var name = genus.toUpperCase();
    ...
    if (month === 1) return letterIndex(genus[0]); // February stops here
    if (name.indexOf("PP") !== -1) return 29;
    if (name.indexOf("RR") !== -1) return 30;
    if (MONTH_DAYS[month] === 30) return letterIndex(genus[0]); // 30-day months stop here
    if (name.indexOf("SS") !== -1) return 31;
    return letterIndex(genus[0]);
  }

  // Hour and minute both come from the genus name: add up the letter
  ...
  //
  // Genus names run much longer than a species epithet ever did (some top
  // 20 letters -- confirmed live: Pseudoblepharispermum, Sibirotrisetokoeleria),
  ...
  // every genus, however long its name is.
  function hourMinuteOf(genus){
    var sum = 0;
    for (var i = 1; i < genus.length; i++) sum += letterIndex(genus[i]);
    var str = String(sum);
  ...
  // Returns null fields when the genus' group has no month yet -- callers
  ...
  // Genera with a real date, precomputed once -- everything that scans by
  ...
  function buildMonthGrid(){
    for (var month = 0; month < 12; month++){
      (function(month){
        ...
        dayGrid.setAttribute("aria-label", MONTH_NAMES[month] + ", " + monthDays + " days, shaded by genus count -- click a day to browse it");
  ...
  function paintHeatmap(){
    MONTHS.forEach(function(m, month){
      ...
      Object.keys(m.cells).forEach(function(d){
        ...
        cell.title = count
          ? MONTH_NAMES[month] + " " + day + " (" + letterForDay(day) + ") — " + count + " genera"
  ...
  function renderResults(result){
    ...
    if (!matches.length){
      ...
      d.textContent = "No genus matches that name.";
  ...
  function buildIcs(r, wikiUrl){
    ...
    var descLines = [r.genus, wikiUrl];

    var uid = encodeURIComponent(r.genus) + "@floweringplantephemeris";
    var lines = [
      "BEGIN:VCALENDAR",
      "VERSION:2.0",
      "PRODID:-//The Flowering Plant Ephemeris//EN",
      "CALSCALE:GREGORIAN",
      "BEGIN:VEVENT",
      "UID:" + uid,
      "DTSTAMP:" + dtstamp,
  ...
  function selectEntry(entry, isAuto){
    ...
    currentWikiUrl = "https://en.wikipedia.org/wiki/" + encodeURIComponent(r.genus);
    loadSpecimenPhoto(r);
  }
  ...
  // is an instant lookup and never lists a genus with no date.
  var DATE_INDEX = [];
  for (var mi = 0; mi < 12; mi++){
    DATE_INDEX[mi] = [];
  ...
  function renderBrowse(month, day){
    ...
    browseSummary.innerHTML = list.length
      ? "<b>" + list.length + "</b> genera share " + MONTH_NAMES[month] + " " + day + " (" + letterForDay(day) + ")."
  ...
  // ---------- Auto-select today's closest genus ----------
  // Show whichever genus lands on today's real month/day (among genera
