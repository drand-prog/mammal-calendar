// Same repo as the mammal admin's lib/github.ts -- the animal family
// apps are a fifth product living in data/animals/ and apps/animals-*/ of
...
export async function commitFaqs(faqs: Faq[]) {
  return commitJsonFile(
    "data/animals/faqs.json",
    faqs,
    `Update animal family FAQs (${faqs.length} question${faqs.length === 1 ? "" : "s"}) via admin panel`
  );
}
...
export async function commitOrders(orders: OrderEntry[]) {
  ...
  return commitJsonFile(
    "data/animals/orders.json",
    orders,
    `Update animal family group→month assignments (${assigned}/${orders.length} groups assigned) via admin panel`
  );
}
...
export async function commitMonthDescriptions(descriptions: string[]) {
  ...
  return commitJsonFile(
    "data/animals/monthDescriptions.json",
    descriptions,
    `Update animal family month descriptions (${filled}/12 filled in) via admin panel`
  );
}
