// Same repo as the mammal admin's lib/github.ts -- the reptile/amphibian
// apps are a third product living in data/reptile/ and apps/reptile-*/ of
...
export async function commitFaqs(faqs: Faq[]) {
  return commitJsonFile(
    "data/reptile/faqs.json",
    faqs,
    `Update reptile/amphibian FAQs (${faqs.length} question${faqs.length === 1 ? "" : "s"}) via admin panel`
  );
}
...
export async function commitOrders(orders: OrderEntry[]) {
  ...
  return commitJsonFile(
    "data/reptile/orders.json",
    orders,
    `Update reptile/amphibian group→month assignments (${assigned}/${orders.length} groups assigned) via admin panel`
  );
}
...
export async function commitMonthDescriptions(descriptions: string[]) {
  ...
  return commitJsonFile(
    "data/reptile/monthDescriptions.json",
    descriptions,
    `Update reptile/amphibian month descriptions (${filled}/12 filled in) via admin panel`
  );
}
