    ...
    return (
      <div className="wrap">
        <div className="login-box">
          <span className="eyebrow">Flowering Plant Ephemeris</span>
          <h1>Admin</h1>
...
function OrdersSection({ initialOrders }: { initialOrders: OrderEntry[] }) {
  ...
  // Genera assigned to each of the 12 months, recomputed as selects change
  // -- lets an admin see at a glance which months are still empty or
  // lopsided (Campanulids and Malvids alone are well over a quarter of all
  // genera) before saving.
  ...
  return (
    <section className="card">
      <h2 className="section-heading">Group months</h2>
      <p className="section-note">
        Flowering plant genera split into a fixed {initialOrders.length} groups by APG clade (Campanulids,
        Lamiales, Malvids, Fabales, Rosales, Poales, Orchids, three catch-all groups for everything else in
        their clade, and Magnoliids + the earliest-diverging lineages — see
        scripts/wcvp_group_mapping_diagnostic.py for exactly how each order maps to a group). Each one needs
        assigning to a month by hand before its genera show up on the public calendar. More than one group
        can share a month.
      </p>

      <p className="orders-summary">
        <b>{assignedOrders}</b> / {initialOrders.length} groups assigned &middot;{" "}
        <b>{assignedSpecies.toLocaleString()}</b> / {totalSpecies.toLocaleString()} genera covered
      </p>
