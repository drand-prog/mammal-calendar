...
"use strict";
  ...
  // ---------- Admin: manage FAQs (real server-side auth + GitHub-backed save) ----------
  // The password check happens in /api/admin/login, server-side, using an
  // env var the browser never sees. This client code only reflects whether
  // that succeeded — it enforces nothing on its own.
  var adminGate = document.getElementById("adminGate");
  var adminEditor = document.getElementById("adminEditor");
  var adminGateStatus = document.getElementById("adminGateStatus");
  var adminEditList = document.getElementById("adminEditList");
  var faqStatus = document.getElementById("faqStatus");

  function setFaqStatus(text, kind){
    faqStatus.textContent = text;
    faqStatus.className = "admin-status" + (kind ? " " + kind : "");
  }

  function buildAdminRow(q, a){
    var row = document.createElement("div");
    row.className = "admin-edit-row";
    row.innerHTML =
      '<label class="admin-label">Question</label>' +
      '<input class="admin-input admin-row-q" type="text" maxlength="200" />' +
      '<label class="admin-label">Answer</label>' +
      '<textarea class="admin-input admin-textarea admin-row-a" maxlength="1000"></textarea>' +
      '<button type="button" class="admin-remove">Remove</button>';
    row.querySelector(".admin-row-q").value = q || "";
    row.querySelector(".admin-row-a").value = a || "";
  ...
  function renderAdminEditor(){
    ...
    FAQS.forEach(function(item){ adminEditList.appendChild(buildAdminRow(item.q, item.a)); });
  }
  ...
  fetch("/api/admin/session")
    ...
    .then(function(d){ if (d && d.loggedIn) showEditor(); })
    .catch(function(){});

  document.getElementById("adminUnlockBtn").addEventListener("click", function(){
    var pass = document.getElementById("adminPass").value;
    adminGateStatus.textContent = "Checking…";
    adminGateStatus.className = "admin-status";
    fetch("/api/admin/login", {
      ...
      .then(function(res){
        if (res.ok){
          ...
          adminGateStatus.textContent = (res.data && res.data.error) || "Incorrect passphrase.";
          adminGateStatus.className = "admin-status err";
        }
      })
      .catch(function(){
        adminGateStatus.textContent = "Couldn't reach the server. Try again.";
        adminGateStatus.className = "admin-status err";
      });
  });
  document.getElementById("adminPass").addEventListener("keydown", function(e){
    if (e.key === "Enter"){ e.preventDefault(); document.getElementById("adminUnlockBtn").click(); }
  });

  document.getElementById("adminAddRow").addEventListener("click", function(){
    var row = buildAdminRow("", "");
    adminEditList.appendChild(row);
    row.querySelector(".admin-row-q").focus();
  });

  document.getElementById("adminSave").addEventListener("click", function(){
    var saveBtn = document.getElementById("adminSave");
    var newFaqs = [];
    adminEditList.querySelectorAll(".admin-edit-row").forEach(function(row){
      var q = row.querySelector(".admin-row-q").value.trim();
      var a = row.querySelector(".admin-row-a").value.trim();
      if (q && a) newFaqs.push({ q: q, a: a });
    });
    ...
    fetch("/api/admin/faqs", {
      ...
      .then(function(res){
        ...
        if (res.ok){
          FAQS = newFaqs;
          renderFaqs();
          setFaqStatus("Saved — pushed a commit to GitHub. Vercel is redeploying; live for everyone in about a minute.", "ok");
        } else if (res.status === 401){
          setFaqStatus("Your session expired — enter the passphrase again.", "err");
          showGate();
        } else {
          setFaqStatus((res.data && res.data.error) || "Couldn't save. Try again.", "err");
        }
      })
      .catch(function(){
        saveBtn.disabled = false;
        setFaqStatus("Couldn't reach the server. Try again.", "err");
      });
  });
  ...
  if (adminLogoutBtn){
    adminLogoutBtn.addEventListener("click", function(){
      fetch("/api/admin/logout", { method: "POST" }).then(function(){ showGate(); });
    });
  }
